...
def test_forecast_adherence():
    ...
    assert response.status_code == 200
    data = response.json()
    assert "predicted_adherence" in data
    assert "adherence_category" in data
    assert 0 <= data["predicted_adherence"] <= 100
    assert data["adherence_category"] in ["Low", "Medium", "High"]
